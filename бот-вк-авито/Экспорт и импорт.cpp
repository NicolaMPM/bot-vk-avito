#include "src/јлгоритмы.cpp"
#include"src/совместимость с вк.cpp"
#include"src/совместимость с авито.cpp"
#include<tuple>
#pragma once
...
namespace ExportAndImportData {
	...
	public:
		vector<tuple<int, string, string>> importDataBaseAvito() {//импорт и поиск людей по базе данных јвито
		...
		vector<tuple<int,string,string>> importDataBaseVK() {//импорт и поиск людей по базе данных ¬ 
			potoc::Vhod *vhod = nullptr;
			MYSQL* link = mysql_init(0);
			vector<tuple<int, string, string>>mass;
			MYSQL_RES* result = mysql_store_result(link);
	...
	public:
		...
		void ExportDataBaseSortVK() {//Ёкспортирование отсортированной и удаление старой базы данных ¬ 
			...
			struct Structures::SortirovkaThree::Three* root = nullptr;
